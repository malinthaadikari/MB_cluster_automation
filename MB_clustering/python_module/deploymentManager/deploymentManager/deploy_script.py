...
#go each node found in the xml file and allocate instance for that node
for node in doc.xpathEval("//node"):
	server = nova.servers.create(name = node.prop('id'),password=myPassword,image= image.id,flavor= flavor.id ,key_name = myKey,nics = [{'net-id': '34c6cbf5-5647-4210-8979-67e0b3b1f88a' ,'v4-fixed-ip': ''}])
...
#start each instance
#TODO : Current starting order is one after another, have to implement a proper way to define the order
while(j<len(doc.xpathEval("//node"))):
	...
	#wait for the port of the server
